...
def fetch_netbox_inventory(netbox_url: str, api_token: str) -> List[Dict[str, Any]]:
    """Fetches devices, hypervisors, and VMs directly from NetBox REST API."""
    ...
    try:
        ...
        if dev_res.status_code == 200:
            ...
            logger.error(f"Failed to fetch devices: HTTP {dev_res.status_code}")
    except Exception as e:
        logger.error(f"Error connecting to NetBox Devices API: {e}")
        raise RuntimeError(f"NetBox Devices API Error: {str(e)}")

    # 2. Fetch Virtualization VMs
    try:
        ...
        logger.warning(f"Could not query Virtualization API: {e}")
