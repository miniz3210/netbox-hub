...
class BackupInspector:
    ...
    def get_object_data(self, endpoint: str) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of objects (empty list if data not available after restore from DB)
        """
        objects = self.inspect_all_objects()
        if endpoint in objects:
            # After DB restore, the "data" field won't exist (we only store counts/metadata)
            # This is expected behavior - full object data is only available from the live JSON backup
    ...
    def _format_label(self, key: str) -> str:
        ...
        """
        # Remove common prefixes for legacy keys
        key = key.replace("netbox_", "")
        
        # Split by / or _
        parts = re.split(r'[/_]', key)
        
        # Remove category prefix if redundant (e.g., "dcim devices" -> "devices")
        if len(parts) > 1 and parts[0] in ["dcim", "ipam", "extras", "virtualization", "circuits", "tenancy", "users", "wireless"]:
            parts = parts[1:]  # Use the second part (e.g., "devices" from "dcim/devices")
        
        # Handle multi-word compound names separated by hyphens
        # Split each part by hyphens to handle cases like "table-configs" or "config-contexts"
        expanded_parts = []
        for part in parts:
            if '-' in part:
                # Split hyphenated words (e.g., "table-configs" -> ["table", "configs"])
        ...
        for part in expanded_parts:
            ...
            if part.upper() in ["IPAM", "DCIM", "VM", "VMS", "IP", "VLAN", "VRF", "ASN", "API", "URL", "BGP", "NAT"]:
