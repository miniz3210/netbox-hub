...
def infer_vms_from_host(host_name: str, host_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    Logic:
    1. Get the cluster assigned to the host
    2. Query all VMs in that cluster
    ...
    if isinstance(host_data, dict):
        cluster_name = host_data.get('cluster')
        if not cluster_name and 'cluster' in str(host_data.get('raw_data', '')):
            ...
            if isinstance(raw, dict):
                ...
                elif isinstance(cluster_obj, str):
                    cluster_name = cluster_obj
    
    if not cluster_name:
        return []
    
    # Query VMs in that cluster - try multiple approaches
    try:
        ...
        all_vms = get_backup_records_by_type(
            ...
            site=None,
            limit=500  # Increased limit to catch more VMs
        )
        
        if all_vms:
            # Flexible cluster matching - handle different formats
            cluster_lower = cluster_name.lower().strip()
            for vm in all_vms:
                vm_cluster = vm.get('cluster', '')
                if vm_cluster:
                    vm_cluster_lower = vm_cluster.lower().strip()
                    # Match if cluster name appears in VM's cluster field
                    if cluster_lower in vm_cluster_lower or vm_cluster_lower in cluster_lower:
                        vms.append(vm)
        
        # Method 2: If no VMs found, try search with cluster as identifier
        if not vms:
