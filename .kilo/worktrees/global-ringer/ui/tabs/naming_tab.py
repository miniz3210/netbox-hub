...
from utils.pattern_formatter import apply_pattern, extract_variables
...
def render_naming_tab(active_model):
    ...
    # ESXi Network Descriptions
        ...
        with col_a:
            ...
            # Get pattern and extract variables
            pattern = naming_rules.get("esxi_uplink", "<vmnic> - <vSwitch> <Status>")
            variables = extract_variables(pattern)
            
            # Display pattern info
            st.caption(f"Pattern: `{pattern}`")
            
            # Create dynamic input fields based on pattern variables
            input_values = {}
            
            for var in variables:
                var_lower = var.lower()
                
                # Determine input type and defaults based on variable name
                if var_lower in ["status"]:
                    # Radio button for status
                    input_values[var] = st.radio(
                        f"{var}", 
                        ["Active Uplink", "Standby Uplink"], 
                        horizontal=True, 
                        key=f"vmnic_{var}"
                    )
                elif var_lower in ["vmnic", "vmnic_id", "nic", "nic_id"]:
                    # vmnic input with normalization
                    raw_value = st.text_input(
                        f"{var}", 
                        value="", 
                        placeholder="e.g. vmnic0, nic1", 
                        key=f"vmnic_{var}"
                    )
                    if raw_value:
                        input_values[var] = normalize_vmnic(raw_value) if auto_correct else raw_value.strip()
                    else:
                        input_values[var] = ""
                elif var_lower in ["vswitch", "vswitch_id", "switch"]:
                    # vSwitch input with normalization
                    raw_value = st.text_input(
                        f"{var}", 
                        value="", 
                        placeholder="e.g. vSwitch0, vswitch1", 
                        key=f"vmnic_{var}"
                    )
                    if raw_value:
                        input_values[var] = normalize_vswitch(raw_value) if auto_correct else raw_value.strip()
                    else:
                        input_values[var] = ""
                else:
                    # Generic text input for custom variables
                    input_values[var] = st.text_input(
                        f"{var}", 
                        value="", 
            ...
            if all_filled:
                gen_vmnic = apply_pattern(pattern, input_values)
            else:
                # Show placeholder for unfilled variables
                placeholder_values = {k: v if v else f"<{k}>" for k, v in input_values.items()}
                gen_vmnic = apply_pattern(pattern, placeholder_values)
        ...
        with col_b:
            ...
            # Get pattern and extract variables
            pattern = naming_rules.get("esxi_portgroup", "<PortGroup> [<Active_vmnics> Active / <Standby_vmnics> Standby]")
            variables = extract_variables(pattern)
            ...
            for var in variables:
                ...
                # Determine input type and defaults based on variable name
                if var_lower in ["portgroup", "port_group", "pg", "portgroup_name"]:
                    input_values[var] = st.text_input(
                        f"{var}", 
                        value="", 
                        placeholder="e.g. PG-VM Network", 
                        key=f"pg_{var}"
                    ).strip()
                elif var_lower in ["active_vmnics", "active_nics", "active"]:
                    raw_value = st.text_input(
                        f"{var}", 
                        value="", 
                        placeholder="e.g. vmnic0, vmnic1", 
                        key=f"pg_{var}"
                    )
                    if raw_value:
                        input_values[var] = normalize_vmnic_list(raw_value) if auto_correct else raw_value.strip()
                    else:
                        input_values[var] = ""
                elif var_lower in ["standby_vmnics", "standby_nics", "standby"]:
                    raw_value = st.text_input(
                        f"{var} (Optional)", 
                        value="", 
                        placeholder="e.g. vmnic2", 
                        key=f"pg_{var}"
                    )
                    if raw_value:
                        input_values[var] = normalize_vmnic_list(raw_value) if auto_correct else raw_value.strip()
            ...
            # Generate output
            # Check if required fields are filled (standby is typically optional)
            required_vars = [v for v in variables if "standby" not in v.lower()]
            required_filled = all(input_values.get(v, "") for v in required_vars)
            
            if required_filled:
                gen_pg = apply_pattern(pattern, input_values)
                # Clean up empty optional placeholders
                import re
                gen_pg = re.sub(r'\s*/\s*<[^>]+>\s*Standby\]', ']', gen_pg)
                gen_pg = re.sub(r'\s*/\s*Standby\]', ']', gen_pg)
                gen_pg = re.sub(r'\[([^\]]*?)\s*/\s*\]', r'[\1]', gen_pg)
            else:
                # Show placeholder for unfilled variables
                placeholder_values = {k: v if v else f"<{k}>" for k, v in input_values.items()}
                gen_pg = apply_pattern(pattern, placeholder_values)
        ...
        with col_c:
            ...
            # Get pattern and extract variables
            pattern = naming_rules.get("esxi_vmkernel", "<Purpose> [<vSwitch>]")
            variables = extract_variables(pattern)
            ...
            for var in variables:
                ...
                # Determine input type and defaults based on variable name
                if var_lower in ["purpose", "service", "role"]:
                    input_values[var] = st.text_input(
                        f"{var}", 
                        value="", 
                        placeholder="e.g. Management, vMotion, Storage", 
                        key=f"vmk_{var}"
                    ).strip()
                elif var_lower in ["vswitch", "vswitch_id", "switch"]:
                    # vSwitch input with normalization
            ...
            # Generate output - show placeholders for empty fields
            placeholder_values = {k: v if v else f"<{k}>" for k, v in input_values.items()}
            fallback_disp = apply_pattern(pattern, placeholder_values)
