...
def render_standards_tab(active_model):
        ...
        with st.form("naming_standards_form"):
            ...
            with col5:
                esxi_host = st.text_area(
                    ...
                    height=80,
                    help="Format: <Site><Role><Seq>.<Domain> (e.g., ageesx001.eswine.adds, nycotinfhost1.eswines.ot)",
                    key="form_esxi"
                )
            ...
            with col6:
                esxi_uplink = st.text_input(
                    ...
                    help="Available: <vmnic>, <vSwitch>, <Status>",
                    key="form_esxi_uplink",
                    autocomplete="off"
                )
                esxi_portgroup = st.text_input(
                    "ESXi Port Group Teaming",
                ...
                esxi_vmkernel = st.text_input(
                    ...
                    help="Available: <Purpose>, <vSwitch>",
                    key="form_esxi_vmk",
                    autocomplete="off"
                )
    ...
    with tab3:
        ...
        with st.expander("🔗 Switch Uplink Description Variables", expanded=True):
            st.markdown("""
**Available Variables:**
...
- `<Role_Zone>_VLAN<VLAN_ID>` → `TRUST_VLAN10`
        ...
        with st.expander("🔗 ESXi Physical Uplink Variables"):
            st.markdown("""
**Available Variables:**
...
- `<vmnic>_<vSwitch>_<Status>` → `vmnic0_vSwitch0_Active_Uplink`
            """)
        
        with st.expander("🔗 ESXi Port Group Teaming Variables"):
            st.markdown("""
**Available Variables:**
- `<PortGroup>` - The port group name (e.g., PG-VM Network)
- `<Active_vmnics>` - Active vmnic list (e.g., vmnic0, vmnic1)
- `<Standby_vmnics>` - Standby vmnic list (optional)

**Example Patterns:**
- `<PortGroup> [<Active_vmnics> Active / <Standby_vmnics> Standby]`
- Result: `PG-VM Network [vmnic0, vmnic1 Active / vmnic2 Standby]`
            """)
...
**Example Patterns:**
- `<Purpose> [<vSwitch>]` → `Management [vSwitch0]`
- `VMK_<Purpose>_<vSwitch>` → `VMK_Management_vSwitch0`
            """)
        ...
        st.markdown("""
You can add your own custom variables to naming patterns. The pattern formatter will replace any `<Variable_Name>` with values you provide.

**How to add custom variables:**
1. Edit your pattern in the **Edit Standards** tab
2. Add your variable using angle brackets: `<Your_Variable>`
3. The Naming tab will show the variable as a placeholder
4. Users can then fill in the values when generating names

**Example Custom Pattern:**
- Pattern: `<Site>_<Building>_<Floor>_<Device_Type>_<Number>`
- Would allow users to create: `NYC_BuildingA_Floor3_Switch_01`
        """)
...
- IT domain: .eswine.adds
- OT domain: .eswines.ot
"""
