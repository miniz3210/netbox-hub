...
def render_naming_tab(active_model):
    ...
    # ESXi Network Descriptions
        ...
        with col_a:
            ...
            gen_vmnic = apply_pattern(pattern, {
                "vmnic": clean_vmnic if clean_vmnic else "vmnic",
                "vSwitch": clean_vsw1 if clean_vsw1 else "vSwitch",
                "Status": status
            })
        ...
        with col_b:
            ...
            if clean_stb:
                gen_pg = apply_pattern(pattern, {
                    "PortGroup": clean_vsw_pg if clean_vsw_pg else "PG-",
                    "Active_vmnics": clean_act if clean_act else "vmnic",
                    "Standby_vmnics": clean_stb
                })
            else:
                # No standby, adjust pattern
                gen_pg = apply_pattern(pattern, {
                    "PortGroup": clean_vsw_pg if clean_vsw_pg else "PG-",
                    "Active_vmnics": clean_act if clean_act else "vmnic",
                    "Standby_vmnics": ""
                })
                # Clean up the pattern if Standby is empty
                import re
                gen_pg = re.sub(r'\s*/\s*Standby\]', ']', gen_pg)
                gen_pg = re.sub(r'\s+/', '', gen_pg)
        ...
        with col_c:
            ...
            fallback_disp = apply_pattern(pattern, {
                "Purpose": vmk_purp if vmk_purp else "Purpose",
                "vSwitch": clean_vsw_vmk if clean_vsw_vmk else "vSwitch"
            })
