...
### 🚀 New Features
* **Azure VM IP Resolution**: The Azure VMs tab now resolves each VM's IP address from the ingested NetBox backup and shows it in a new `NetBox IP` column, with a `🌐 IP Found in NetBox` counter. `core/azure_vm_importer.py` gains `build_vm_ip_index()`, `lookup_vm_ip_addresses()` and `enrich_vms_with_netbox_ips()`, merging two NetBox sources: the `Primary IP` on each `virtualization/virtual-machines` record and every `ipam/ip-addresses` entry assigned to a VM interface. When a VM has several addresses the primary is shown with a `(+n)` suffix. Resolved IPs are also written into the NetBox record description as `NetBox IP: <addr>`.
...
* **Fictional Sample CSV**: The `Sample Azure VM CSV Format` placeholder shown before an upload now uses invented hostnames, subscriptions and resource groups, and a documentation-range IP (`198.51.100.10`), instead of real production identifiers.
